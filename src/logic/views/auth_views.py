...
def efetuar_ativacao(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        user.is_active = True
        user.save()
        return render(request, 'ativar_conta.html', {'sucesso': True})
    else:
        return render(request, 'ativar_conta.html', {'erro': 'Token inválido.'})
...
def ativar_conta(request, user, email):
    email = EmailMessage("Ative sua conta",
        render_to_string("email_ativacao.html", {
            'user': user.username,
    ...
    if email.send():
        return render(request, 'ativar_conta.html')
    else:
        return render(request, 'ativar_conta.html', {'erro': 'Erro ao enviar email.'})
...
def redefinir_senha(request, uidb64, token):
    ...
    if request.method != 'POST':
        if user is not None and account_activation_token.check_token(user, token):
            return render(request, 'redefinir_senha.html', {'uidb64': uidb64, 'token': token})
        else: # Se o token não for válido, ou o usuário não existir, retorna um erro
            return render(request, 'redefinir_senha.html', {'erro': 'Token inválido.'})
    # Se o método for POST, a senha do usuário é redefinida
    senha_crua = request.POST.get('senha')
    if not senha_crua:
        return render(request, 'redefinir_senha.html', {'erro': 'Preencha todos os campos.'})
    user.password = make_password(senha_crua)
    user.save()
    return render(request, 'redefinir_senha.html', {'sucesso': True})
...
def recuperar_senha(request):
    ...
    if not email:
        return render(request, 'recuperar_senha.html', {'erro': 'Preencha todos os campos.'})
    try:
        user = Usuario.objects.get(email=email)
    except:
        user = None
    if user is None:
        return render(request, 'recuperar_senha.html', {'erro': 'Usuário não encontrado.'})
    email = EmailMessage("Redefinição de senha",
        render_to_string("email_redefinir_senha.html", {
            'user': user.username,
            "protocolo": 'https' if request.is_secure() else 'http',
            'dominio': get_current_site(request).domain,
            'uid': urlsafe_base64_encode(force_bytes(user.pk)),
    ...
    if email.send():
        return render(request, 'recuperar_senha.html', {'sucesso': True})
    else:
        return render(request, 'recuperar_senha.html', {'erro': 'Erro ao enviar email.'})
...
def login(request):
    ...
    if not Usuario.objects.filter(username=request.POST.get('nome'), is_active=True):
        return render(request, 'login.html', {'erro': 'Conta não ativada, verifique seu email.'})
    if not user:
